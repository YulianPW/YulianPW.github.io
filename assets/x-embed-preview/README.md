...
## 目录
    ...
    └── scripts/
        ...
        └── media/
            ...
            ├── playback-controller.js # 全站唯一播放、离屏和后台暂停
...
全站任一时刻只允许一个列表或灯箱视频播放。列表视频可见面积持续低于 25% 时自动暂停，页面进入后台或离开时也立即暂停；暂停保留进度和缓冲，返回视口后由用户点按“继续播放”，不得自动恢复声音。
...
3. 验证图片灯箱、拼图内视频播放、视频放大、前后切换和移动端无横向溢出；列表视频划出视口、切换卡片、打开灯箱或页面进入后台后不得残留声音，返回时应从原进度继续。
