...
因为网站使用了 `fetch()` API 加载 `assets/data/data.json` 和 `assets/fragments/info-content.html`，直接打开文件会因为 CORS 策略而无法正常工作。
...
`version.json` 由 GitHub Actions 构建阶段动态生成在部署产物根目录中，不长期存放在仓库根目录。
